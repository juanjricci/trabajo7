...
class ProductoService:
    ...
    def busqueda_binaria(self, lista, precio_buscado):
        ...
        for i in range(1, len(lista_ordenada)):
            actual = lista_ordenada[i]
            j = i
            while j > 0 and lista_ordenada[j-1]["_precio"] > actual["_precio"]:
                lista_ordenada[j] = lista_ordenada[j-1]
                j = j-1
            # insertar el elemento en su lugar
            lista_ordenada[j] = actual
        ...
        while izq <= der:
            medio = (izq+der)//2
            pmed = lista_ordenada[medio]['_precio']
            if pmed == precio_buscado:
                # print (lista_ordenada[medio])
                return lista_ordenada[medio]
            elif pmed > precio_buscado:
                der = medio-1
            else:
                izq = medio+1
        return -1
